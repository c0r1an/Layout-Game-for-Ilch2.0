...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("game-export-" + [guid]::NewGuid().ToString('N'))
$tempLayoutDir = Join-Path $tempRoot 'game'
...
Compress-Archive -Path (Join-Path $tempRoot 'game') -DestinationPath $zipPath -Force
